...
async def test_proxy(proxy_str: str, check_fraud: bool = True, timeout: int = 8) -> dict:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with s.post(
                    ...
                    stripe_ms = round((time.perf_counter() - t1) * 1000)
                    if resp.status in (200, 400, 401, 402, 403, 404):
                        return {"stripe": True, "stripe_ms": stripe_ms}
        except Exception:
            pass
        return {"stripe": False, "stripe_ms": None}

    # Run Step 1 and Step 3 in parallel to save time
    s1_task = asyncio.create_task(run_step1())
    s3_task = asyncio.create_task(run_step3())

    s1_res = await s1_task
    
    # If Step 1 fails, the proxy is dead. Cancel Step 3 and return.
    if not s1_res["success"]:
        s3_task.cancel()
        result["error"] = s1_res["error"]
        return result
    ...
    result["ms"] = s1_res["ms"]
    result["ip"] = s1_res["ip"]
    result["country"] = s1_res["country"]
    result["country_code"] = s1_res["country_code"]
    result["isp"] = s1_res["isp"]
    if s1_res["hosting"]:
        result["type"] += " (DC)"
    else:
        result["type"] += " (Resi)"
    ...
    if check_fraud and result["ip"]:
        try:
            # We keep a very short timeout for fraud checks to avoid blocking the main path
    ...
    s3_res = await s3_task
    result["stripe"] = s3_res["stripe"]
    result["stripe_ms"] = s3_res["stripe_ms"]
    
    return result


async def test_proxies_batch(proxies: list, concurrency: int = 10, check_fraud: bool = True, timeout: int = 8) -> list:
    sem = asyncio.Semaphore(concurrency)
    async def _run(p):
        async with sem:
            return await test_proxy(p, check_fraud=check_fraud, timeout=timeout)
...
async def cmd_proxy(msg: Message, command: CommandObject, bot: Bot):
    ...
    if sub == "add":
        await _add_proxies(msg, uid, rest)
    elif sub in ("del", "rm", "remove"):
        await _del_proxy(msg, uid, rest)
    elif sub == "clear":
...
async def handle_document_proxy(msg: Message, bot: Bot):
    uid = msg.from_user.id
    await db.upsert_user(uid, msg.from_user.username, msg.from_user.first_name)
    if await db.is_banned(uid):
        return
    ...
    # Process if it is a .txt file
    doc = msg.document
    if not doc.file_name or not doc.file_name.lower().endswith(".txt"):
        return

    status_msg = await msg.answer(
        f"{EMOJI['hitting']} Downloading proxy file...",
    ...
    try:
        ...
        if not lines:
            await status_msg.edit_text(f"{EMOJI['declined']} The uploaded text file is empty.", parse_mode=ParseMode.HTML)
            return

        await status_msg.edit_text(
            f"{EMOJI['hitting']} Testing {len(lines)} proxy(s) from file...\n"
            f"Checking IP, country, speed, and Stripe...",
            parse_mode=ParseMode.HTML
        )

        # Batch test the proxies (15s timeout to prevent slow proxies from timing out)
        results = await test_proxies_batch(lines, concurrency=80, check_fraud=False, timeout=15)
        alive, dead = [], []
        for r in results:
            if r["alive"]:
        ...
        dead_lines = "\n".join(f"{EMOJI['declined']} <code>{r['proxy']}</code>" for r in dead[:3])
        if len(alive) > 5:
        ...
        # Send txt file back with working ones
        if alive:
            from aiogram.types import BufferedInputFile
            file_content = "\n".join(r["proxy"] for r in alive)
            file_bytes = file_content.encode("utf-8")
            input_file = BufferedInputFile(file_bytes, filename="working_proxies.txt")
            await msg.answer_document(
                input_file,
                caption=f"📄 Saved {len(alive)} working proxies from your file to your account."
            )
            
    except Exception as e:
        await status_msg.edit_text(f"{EMOJI['declined']} Failed to process file: {str(e)[:50]}", parse_mode=ParseMode.HTML)
...
async def _show_proxy_list(msg: Message, uid: int):
    ...
    text = (
        f"<blockquote>🌐 <b>Proxy Manager</b> 🌐 ❞</blockquote>\n\n"
        f"<blockquote>📡 <b>[•] Your Proxies</b> ({len(proxies)}):\n{lines}</blockquote>\n\n"
        f"<blockquote>🛠️ <b>[•] Commands</b>\n"
        f"➜ <code>/proxy add host:port:user:pass</code>\n"
        f"➜ <code>/proxy del &lt;proxy&gt;</code>\n"
        f"➜ <code>/proxy clear</code>\n"
        f"➜ <code>/proxy test</code> ─ Full check (IP, country, speed, Stripe)</blockquote>\n\n"
        f"<blockquote>💡 <i>Formats: host:port | host:port:user:pass | user:pass@host:port</i> ❞</blockquote>"
    )
...
async def _add_proxies(msg: Message, uid: int, text: str):
    ...
    # Use concurrency=80 and timeout=15 for reliable DNS & socket allocation on bulk checks
    results = await test_proxies_batch(lines, concurrency=80, check_fraud=False, timeout=15)
    alive, dead = [], []
    for r in results:
        if r["alive"]:
    ...
    output = (
        f"<blockquote>📡 <b>Proxy Add</b> 📡 ❞</blockquote>\n\n"
        f"<blockquote>🟢 <b>[•] Alive</b> → <code>{len(alive)}/{len(lines)}</code> ❞\n"
        f"🔴 <b>[•] Dead</b> → <code>{len(dead)}/{len(lines)}</code></blockquote>\n\n"
    )
    if alive_blocks:
        output += "<blockquote><b>Added:</b>\n\n" + "\n\n".join(alive_blocks) + "</blockquote>\n\n"
    if dead_lines:
        output += f"<blockquote><b>Failed:</b>\n{dead_lines}</blockquote>"
    ...
    # If there are working proxies, send them in a txt file
    if alive:
        try:
            from aiogram.types import BufferedInputFile
            # Include all working proxies, one per line
            file_content = "\n".join(r["proxy"] for r in alive)
            file_bytes = file_content.encode("utf-8")
            input_file = BufferedInputFile(file_bytes, filename="working_proxies.txt")
            await msg.answer_document(
                input_file,
                caption=f"📄 Here are all {len(alive)} working proxies added to your bot account."
            )
        except Exception as e:
            import logging
            logging.getLogger(__name__).error(f"Error sending proxies text file: {e}")
...
async def _test_proxies(msg: Message, uid: int):
    ...
    results = await test_proxies_batch(proxies, concurrency=80, check_fraud=True, timeout=15)
    ...
    for r in results[:10]:
        if r["alive"]:
            ...
            err_msg = "Connection Failed" if r['error'] == "Timeout" else r['error']
            blocks.append(
    ...
    text = (
        f"<blockquote>📡 <b>Proxy Check</b> 📡 ❞</blockquote>\n\n"
        f"<blockquote>📊 <b>[•] Total</b> → <code>{len(proxies)}</code> ❞\n"
        f"🟢 <b>[•] Alive</b> → <code>{len(alive)}</code>\n"
        f"🔴 <b>[•] Dead</b> → <code>{len(dead)}</code>\n"
        f"💳 <b>[•] Stripe OK</b> → <code>{len(stripe_ok)}/{len(alive)}</code></blockquote>\n\n"
        + "\n\n".join(f"<blockquote>{b}</blockquote>" for b in blocks)
    )
    if len(proxies) > 10:
        text += f"\n\n<blockquote><i>... and {len(proxies)-10} more</i> ❞</blockquote>"
    text += "\n\n<blockquote><i>Fraud data by proxycheck.io</i> ❞</blockquote>"
...
async def cmd_ipcheck(msg: Message, command: CommandObject):
    ...
    try:
        ...
        text = (
            f"<blockquote>🔍 <b>IP Check</b> 🔎 ❞</blockquote>\n\n"
            f"<blockquote>📡 <b>[•] IP</b> → <code>{ip_to_check}</code> ❞\n"
            f"🌍 <b>[•] Country</b> → <code>{country} ({cc})</code>\n"
            f"🏢 <b>[•] ISP</b> → <code>{isp}</code>\n"
            f"💻 <b>[•] Type</b> → <code>{ip_type}</code>{flag_str}</blockquote>\n\n"
            f"<blockquote>🛡️ <b>[•] Fraud Score</b> → {fraud_line} ❞\n"
            f"💳 <b>[•] Stripe</b> → {stripe_line}</blockquote>\n\n"
        )
        
        # Verdict
        if stripe_ok and (fs is None or fs <= 50):
            text += f"<blockquote>🟢 <b>GOOD for hitting</b> ❞</blockquote>"
        elif stripe_ok and fs and fs <= 75:
            text += f"<blockquote>🟡 <b>Usable but risky</b> ❞</blockquote>"
        elif not stripe_ok:
            text += f"<blockquote>🔴 <b>Cannot reach Stripe</b> ❞</blockquote>"
        else:
            text += f"<blockquote>🔴 <b>High fraud score — likely blocked</b> ❞</blockquote>"
...
async def _check_proxy_only(msg: Message, uid: int, proxy_str: str):
    ...
    text = (
        f"<blockquote>🔍 <b>Proxy Check</b> 🔎 ❞</blockquote>\n\n"
        f"<blockquote>📡 <b>[•] Proxy</b> → <code>{html.escape(proxy_str)}</code> ❞\n"
        f"🌐 <b>[•] IP</b> → <code>{html.escape(str(r['ip']))}</code>\n"
        f"🌍 <b>[•] Country</b> → <code>{html.escape(str(r['country']))} ({html.escape(str(r['country_code']))})</code>\n"
        f"🏢 <b>[•] ISP</b> → <code>{html.escape(str(r['isp']))}</code>\n"
        f"💻 <b>[•] Type</b> → <code>{html.escape(str(r['type']))}</code></blockquote>\n\n"
        f"<blockquote>⚡ <b>[•] Latency</b> → <code>{r['ms']}ms</code> ❞\n"
        f"🛡️ <b>[•] Fraud</b> → {fraud_line}\n"
        f"💳 <b>[•] Stripe</b> → {stripe_s}</blockquote>\n\n"
        f"<blockquote><b>Save this proxy to your account?</b> ❞</blockquote>"
    )
