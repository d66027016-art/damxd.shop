...
from functions.epoch_tls import get_checkout_info, charge_card
...
def extract_checkout_url(text: str) -> str:
    for pattern in [
        r"https?://wnu\.com/secure/services/[^\s\"\'<>)]+",
        r"https?://[\w.-]*wnu\.com/[^\s\"\'<>)]+",
        r"https?://[\w.-]*epoch\.com/[^\s\"\'<>)]+",
    ]:
...
WATERMARK = f"{EMOJI['bolt']} 𝗘𝗣𝗢𝗖𝗛 𝗛𝗜𝗧𝗧𝗘𝗥 {EMOJI['bolt']}"
...
# Stop callback
@router.callback_query(F.data.startswith("epoch_stop_hit_"))
...
# Saved BIN callbacks
@router.callback_query(F.data.startswith("epochsbin_cancel_"))
...
@router.callback_query(F.data.startswith("epochsbin_"))
async def cb_sbin_select(query: CallbackQuery, bot: Bot):
    ...
    stop_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Stop", callback_data=f"epoch_stop_hit_{uid}")]])
...
# /hit command
@router.message(Command("ep", prefix="/."))
async def cmd_ep(msg: Message, bot: Bot):
    uid = msg.from_user.id
    
    gw_status = await db.get_setting("gateway_ep_status", "off")
    is_owner = uid in OWNER_IDS or await db.is_db_owner(uid)
    if gw_status == "off" and not is_owner:
        await msg.answer(f"{EMOJI['error']} <b>Gateway EP is now turned ON FOR OWNER ONLY</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if not url:
        await msg.answer(
            f"Usage:\n<code>/ep &lt;epoch-wnu-url&gt; cc|mm|yy|cvv</code>",
            parse_mode=ParseMode.HTML, link_preview_options=NO_PREVIEW
        )
        return

    remaining = text
    remaining = re.sub(r"^[./]ep\s*", "", remaining, flags=re.IGNORECASE).strip()
    ...
    if not cards:
        if url:
            ...
            if saved_bins:
                buttons = [[InlineKeyboardButton(text=f"{b['name']}", callback_data=f"epochsbin_{uid}_{b['name']}")] for b in saved_bins[:10]]
                buttons.append([InlineKeyboardButton(text="Cancel", callback_data=f"epochsbin_cancel_{uid}")])
...
async def _run_hit(msg, bot, uid, cards, checkout, url, proxy, status_msg, amount_display, merchant, success_url):
    ...
    for i, card in enumerate(cards):
        ...
        if (now_ts - last_edit) >= 1.0 or i == 0:
            hitting_text = (
                f"<blockquote>⚡ <b>Epoch Checker</b> ⚡ ❞</blockquote>\n\n"
                f"<blockquote>🏪 <b>[•] Merchant</b> → <code>{merchant}</code> ❞\n"
                f"💰 <b>[•] Amount</b> → <code>{amount_display}</code></blockquote>\n\n"
