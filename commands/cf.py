...
from functions.cashfree_tls import get_checkout_info, charge_card
...
def extract_checkout_url(text: str) -> str:
    for pattern in [
        r"https?://cfp\.io/l/[^\s\"\'<>)]+",
        r"https?://pages\.cashfree\.com/[^\s\"\'<>)]+",
        r"https?://checkout\.cashfree\.com/[^\s\"\'<>)]+",
        r"https?://payments\.cashfree\.com/[^\s\"\'<>)]+",
        r"https?://api\.cashfree\.com/[^\s\"\'<>)]+",
    ]:
        m = re.search(pattern, text, re.IGNORECASE)
        if m:
            return m.group(0).rstrip(".,;:")
    return "give a valid cashfree checkout url (https://payments.cashfree.com/checkout/[some_random_string])"
...
WATERMARK = f"{EMOJI['bolt']} cashfree 𝗛𝗜𝗧𝗧𝗘𝗥 {EMOJI['bolt']}"
...
# Stop callback
@router.callback_query(F.data.startswith("cf_stop_hit_"))
...
# Saved BIN callbacks
@router.callback_query(F.data.startswith("cfsbin_cancel_"))
...
@router.callback_query(F.data.startswith("cfsbin_"))
async def cb_sbin_select(query: CallbackQuery, bot: Bot):
    ...
    stop_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Stop", callback_data=f"cf_stop_hit_{uid}")]])
...
# /hit command
@router.message(Command("cf", prefix="/."))
async def cmd_cf(msg: Message, bot: Bot):
    uid = msg.from_user.id
    
    gw_status = await db.get_setting("gateway_cf_status", "off")
    is_owner = uid in OWNER_IDS or await db.is_db_owner(uid)
    if gw_status == "off" and not is_owner:
        await msg.answer(f"{EMOJI['error']} <b>Gateway CF is now turned ON FOR OWNER ONLY</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if not url:
        await msg.answer(
            f"Usage:\n<code>/cf &lt;cashfree-url&gt; cc|mm|yy|cvv</code>",
    ...
    if not cards:
        if url:
            ...
            if saved_bins:
                buttons = [[InlineKeyboardButton(text=f"{b['name']}", callback_data=f"cfsbin_{uid}_{b['name']}")] for b in saved_bins[:10]]
                buttons.append([InlineKeyboardButton(text="Cancel", callback_data=f"cfsbin_cancel_{uid}")])
    ...
    if checkout.get("error"):
        ...
        if is_privileged:
            ...
            for i in range(total_fake):
                ...
                hitting_text = (
                    f"<blockquote>⚡ <b>Cashfree Checker</b> ⚡ ❞</blockquote>\n\n"
                    f"<blockquote>🏪 <b>[•] Merchant</b> → <code>Premium Gateway</code> ❞\n"
