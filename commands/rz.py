...
from functions.razorpay_tls import get_checkout_info, charge_card
...
WATERMARK = f"{EMOJI['bolt']} 𝗥𝗔𝗭𝗢𝗥𝗣𝗔𝗬 𝗛𝗜𝗧𝗧𝗘𝗥 {EMOJI['bolt']}"
...
# Stop callback
@router.callback_query(F.data.startswith("rz_stop_hit_"))
...
# Saved BIN callbacks
@router.callback_query(F.data.startswith("rzsbin_cancel_"))
...
@router.callback_query(F.data.startswith("rzsbin_"))
async def cb_sbin_select(query: CallbackQuery, bot: Bot):
    ...
    stop_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Stop", callback_data=f"rz_stop_hit_{uid}")]])
...
# /hit command
@router.message(Command("rz", prefix="/."))
async def cmd_rz(msg: Message, bot: Bot):
    uid = msg.from_user.id
    
    gw_status = await db.get_setting("gateway_rz_status", "off")
    is_owner = uid in OWNER_IDS or await db.is_db_owner(uid)
    if gw_status == "off" and not is_owner:
        await msg.answer(f"{EMOJI['error']} <b>Gateway RZ is now turned ON FOR OWNER ONLY</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if not url:
        await msg.answer(
            f"Usage:\n<code>/rz &lt;razorpay-url&gt; cc|mm|yy|cvv</code>",
    ...
    if not cards:
        if url:
            ...
            if saved_bins:
                buttons = [[InlineKeyboardButton(text=f"{b['name']}", callback_data=f"rzsbin_{uid}_{b['name']}")] for b in saved_bins[:10]]
                buttons.append([InlineKeyboardButton(text="Cancel", callback_data=f"rzsbin_cancel_{uid}")])
    ...
    if checkout.get("error"):
        ...
        if is_privileged:
            ...
            for i in range(total_fake):
                ...
                hitting_text = (
                    f"<blockquote>⚡ <b>Razorpay Checker</b> ⚡ ❞</blockquote>\n\n"
                    f"<blockquote>🏪 <b>[•] Merchant</b> → <code>Premium Gateway</code> ❞\n"
